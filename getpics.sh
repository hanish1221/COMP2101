...
# download a tgz file of pictures to our Pictures directory if it isn't already there - assumes you are online
test -f ~/Pictures/pics.tgz || wget -q -O ~/Pictures/pics.tgz https://zonzorp.net/pics.tgz

# unpack the downloaded tgz file if it is there, then delete the local copy of the zipfile
test -f ~/Pictures/pics.tgz && tar -C ~/Pictures -xvf ~/Pictures/pics.tgz && rm -f ~/Pictures/pics.tgz
