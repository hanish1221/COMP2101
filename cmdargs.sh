...
# TASK 3: put anything that wasn't recognized on the command line into a variable for use later in the script
 case $1 in
	-v | --verbose)
	 shift
	 echo "Found Verbose Mode on Command Line"
	 exit
	 ;;
	-h | --help)
	 shift
	 echo "Found Help on Command Line"
	 exit
	 ;;
	-d | --debug)
	 if [ ! -z $2 ] && [ $2 -ge 0 ]
	 then
	  level=$2
	 echo "Found Debug Mode on Command Line with level=$level"
	 shift
	 exit
	 else
	 echo "Debug Mode on Command Line Requires a Level"
	 shift
	 exit
	 fi
	 ;;
	* )
	shift
	useafter=$1
	echo "$useafter not recognised option "
	exit
esac
  $echo "Processing '$1'."
